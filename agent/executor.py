...
def _inject_context(params: dict, tool: str, step_results: dict, goal: str = "") -> dict:
    """
    Multi-turn chaining: inject results from previous steps into current step params.
    Handles: file write content, search queries refined by prior results, and generic injection.
    """
    if not step_results:
        return params
    ...
    prior_results = [
        ...
        if v and isinstance(v, str) and len(v) > 30 and v not in ("Done.", "Completed.", "Task completed successfully.")
    ]

    # Chain into file write: fill content with accumulated research
    if tool == "file_controller" and params.get("action") in ("write", "create_file"):
        ...
        if not content_val or len(content_val) < 50:
            if prior_results:
                ...
                print(f"[Executor] 💉 Chained {len(prior_results)} results into file content")

    # Chain search: if a web_search step follows another, include prior summary as context
    elif tool == "web_search" and prior_results:
        existing_query = params.get("query", "")
        # Append context signal so the search is more targeted
        if existing_query and len(prior_results) == 1:
            summary = prior_results[0][:200]
            params["_chain_context"] = summary   # passed to web_search for refined results
            print(f"[Executor] 💉 Chained search context into web_search")

    # Chain into code_helper: if there's prior web_search content, inject as context
    elif tool in ("code_helper", "dev_agent") and prior_results:
        desc = params.get("description", "")
        if desc and prior_results:
            chain_ctx = "\n".join(f"- {r[:300]}" for r in prior_results[:3])
            params["description"] = f"{desc}\n\n[Context from prior steps]:\n{chain_ctx}"
            print(f"[Executor] 💉 Chained context into {tool}")
